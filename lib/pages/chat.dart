...
import 'package:ume_connect/widgets/errorMessage.dart';
import 'package:ume_connect/widgets/glowingActionButton.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    super.initState();
    StreamChannel.of(context).queryMembers().then((result) {
      print(result);
    });
  ...
  _joinMeeting() async {
    ...
    String otherPerson = Helpers.getChannelUID(channel, context.currentUser!)!
        .replaceAll(new RegExp(r'[^\w\s]+'), '');
    List join = [otherPerson, context.currentUser!.id];
    join.sort();
    join.join();
    String roomName;
    roomName = join.join();
    print("ROOM NAME = " + roomName);
    print("CURRENT UID " + context.currentUser!.id);
    print("OTHER UID" +
        Helpers.getChannelUID(channel, context.currentUser!)!
            .replaceAll(new RegExp(r'[^\w\s]+'), ''));
    ...
    var options = JitsiMeetingOptions(room: otherPerson)
      ..serverURL = null
      ..subject = null
      ..userDisplayName = (firebaseuser!.displayName!).substring(0, 1)
      ..userEmail = null
